...
7. Fast path: set `MNR_TELEMETRY_SECRET` in the local PowerShell environment without putting it in Git, logs or chat; run `scripts/release-local.ps1 -Version X.Y.Z`, inspect `Releases/vX.Y.Z`, then run the same command with `-Publish`. Preparation requires a clean private checkout whose tree equals the pushed public `release/main`, reruns `verify.ps1` and builds/packages host/UI. Publication verifies the prepared files and their source commit, uploads a draft GitHub Release, compares every uploaded SHA-256 and makes it public without rebuilding. Never prepare without the telemetry secret: that would disable installed reporting. The secret is currently only configured in the public GitHub Actions repository, so local fast releases need a one-time secure local setup.
